...
//				echo $_POST['unfollow_rand_id'];
		...
		$req_id = mb_substr($now_url,8);//URLからユーザーIDを特定
		//ランダムIDの取得
...
//			echo"</pre>";
		...
		//ユーザー数のカウント
		$sql = "select count(*) from follow_list where rand_id_me=:id";	//ユーザーが存在するか？
		$stmt = $pdo->prepare($sql); //挿入する値は空のまま、SQL実行の準備をする
...
//			echo"</pre>";
		...
		//情報を取得
		$data_f_list = $pdo -> query("select * from follow_list where rand_id_me=\"".$req_rand_id."\" order by follow_list.unix_time DESC");	//ユーザーが存在するか？
	?>
</head>
			...
			<div class="top_form">
				<div class="follow_me_profile">
					...
					<div class="follow_me_yoko">
						<h1 class="index_title"><a class="profile_no_under_a" href="/u&quest;<?php echo $data_reqid['id']; ?>"><?php echo $displayname_req ?></a>がフォロー中</h1>
						<p class="desp1 desp1_mobile"><?php if($data_sonzai[0] == 0){ echo "まだ誰もフォローしていません";}else{ echo $data_sonzai[0]." 人をフォローしています";} ?></p>
					</div>
				</div>
...
//						echo "</pre>";
						foreach($data_f_list as $row){
							...
							$stmt = $pdo->prepare($sql); //挿入する値は空のまま、SQL実行の準備をする
							$params = array(':rand_id' => $row['rand_id_aite']); // 挿入する値を配列に格納する
							$res=$stmt->execute($params); //挿入する値が入った変数をexecuteにセットしてSQLを実行
							if($res){
								$data = $stmt->fetch();
//								echo"<pre>";
